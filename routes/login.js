  ...
  const result = adminSchema
    .findOne({ email }, (err, data) => {
      if (err) {
        ...
        const searchResult = adminSchema
          .findOneAndUpdate(
            { email },
            ...
            (err, data) => {
              if (err) {
                ...
                res
                  .status(400)
                  .send("Could not find this email in DB to update");
              }
            }
          )
          .lean();
...
router.put("/login/password", (req, res) => {
  ...
  const result = adminSchema
    .findOneAndUpdate({ email }, { password: newPassword }, (err, data) => {
      if (err) {
        res.status(400).send("Unexpected error occured");
      } else if (data) {
